...
export class QualityValidator {
  run(g: {
    ...
    if (!g.questions?.length || g.questions.length < 2) issues.push('few_questions');
    if (g.questions && g.questions.length > 10) issues.push('too_many_questions');
    ...
    g.questions?.forEach((q, i) => {
      ...
      if (q.choices && q.choices.length > 6) issues.push(`q${i}_too_many_choices`);
    ...
    const blockers = issues.filter(x =>
