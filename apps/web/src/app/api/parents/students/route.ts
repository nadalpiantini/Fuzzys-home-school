...
export async function GET(request: NextRequest) {
  try {
    ...
    if (profileError || !parentProfile) {
      return NextResponse.json({ error: 'Access denied - not a parent' }, { status: 403 });
    }
    ...
    const studentsPromises = studentIds.map(async (studentId) => {
      ...
      const [pointsData, streakData, lastActivityData, subjectsData] = await Promise.all([
      ...
      const lastActivity = lastActivityData.data?.[0]?.started_at || profile.created_at;
      const subjects = [...new Set(subjectsData.data?.map(s => s.subject) || ['Matemáticas', 'Lengua', 'Ciencias'])];
    ...
    const students = (await Promise.all(studentsPromises)).filter(Boolean) as Student[];
