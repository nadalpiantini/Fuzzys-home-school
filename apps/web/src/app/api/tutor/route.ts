...
import { buildSystemPrompt, buildWelcomeMessage, buildErrorEncouragement, TutorContext } from "@/lib/tutor/compose";
...
export async function POST(req: Request) {
  try {
    ...
    if (action === 'send_query') {
      ...
      const response = await deepseekClient.generateResponse(
        messageHistory,
        {
      ...
      // Telemetría ligera
      console.log(`[TUTOR] Reply generated for session ${sessionId}, persona: fuzzy-v1-${session.context.idioma}`);
