...
export async function GET(req:Request){ try{
  ...
  const game=(sp.get('game')||'').toLowerCase();
  let list=CATALOG;
  if(type) list=list.filter(x=>(x.type||'').toLowerCase()===type);
  if(game) list=list.filter(x=>(x.game||'').toLowerCase()===game || x.id.toLowerCase()===game || (x.tags||[]).some(t=>t.toLowerCase()===game));
  return NextResponse.json({ok:true,data:list});
}catch(e:any){ return NextResponse.json({ok:false,error:e?.message??'Server error'},{status:500});}}
export async function POST(req:Request){ try{
  const supabase=getSupabaseServer(false); const body=await req.json();
