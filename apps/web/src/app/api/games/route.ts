...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!gameId || !answers) {
      return NextResponse.json({ error: 'Game ID and answers are required' }, { status: 400 })
    }
...
function generateGame(type: string | null, subject: string | null, level: string | null) {
  const gameTypes = {
    dragdrop: {
...
function calculateGameScore(gameId: string, answers: any) {
  ...
  return {
    ...
    feedback: mockScore >= 8 ? 'Excellent!' : mockScore >= 6 ? 'Good job!' : 'Keep practicing!',
